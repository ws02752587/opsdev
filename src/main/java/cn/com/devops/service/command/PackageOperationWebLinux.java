...
public class PackageOperationWebLinux extends PackageOperationWeb {
    @Value("${sh.url}")
    ...
    public boolean packge(Deploy deploy) {
        ...
        try {
            Process process = Runtime.getRuntime().exec(sh_url+"packageTar.sh",
                    new String[]{url, name, env.getWebIp()});
            process.waitFor();
            ...
            if(str.contains(App.SH_SUCCESS)){
                flag = true;
                deploy1.setAppStatus(DeployWebEnum.READY.toString());
            }else{
                flag = false;
                deploy1.setErrorMsg(str);
                deploy1.setAppStatus(DeployWebEnum.PACKAGE_FAIL.toString());
            }
