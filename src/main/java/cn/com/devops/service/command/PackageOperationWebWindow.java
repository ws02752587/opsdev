...
public class PackageOperationWebWindow extends PackageOperationWeb {
	private Logger logger = LoggerFactory.getLogger(PackageOperationWebWindow.class);
	@Value("${sh.url}")
    ...
    public boolean packge(Deploy deploy) {
        ...
        if("success".contains(App.SH_SUCCESS)){
            flag = true;
            deploy1.setWebStatus(DeployWebEnum.READY.toString());
        }else{
            flag = false;
            deploy1.setErrorMsg("error");
            deploy1.setWebStatus(DeployWebEnum.PACKAGE_FAIL.toString());
        }
        deployService.update(deploy1);
        logger.info("sh_url:{}", sh_url);
        logger.info("web url:{} name:{} envAppIp:{}",url,name,env.getWebIp());
        return flag;
    }
